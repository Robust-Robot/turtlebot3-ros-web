...
ROS2D.PolygonMarker.prototype.createPointShape = function(pos) {
	...
	point.x = pos.x;
	point.y = -pos.y;
	var r = this.pointSize;
	//point.setBounds(pos.x-r, -pos.y-r, 2*r, 2*r); // So that getBounds() can be used on the polygon
	//point.setBounds(-r, -r, 2*r, 2*r); // So that getBounds() can be used on the polygon
	point.setBounds(0, 0, 0, 0); // So that getBounds() can be used on the polygon
 ...
 *
 */
ROS2D.PolygonMarker.prototype.clear = function() {
 ...
 * @param bool
 */
ROS2D.PolygonMarker.prototype.linesVisible = function(visible) {
	this.lineContainer.visible = visible;
};

/**
 * Set visibility of the points
 *
 * @param bool
 */
ROS2D.PolygonMarker.prototype.pointsVisible = function(visible) {
	this.pointContainer.visible = visible;
};
 ...
 *
 */
ROS2D.PolygonMarker.prototype.getSize = function() {
	return this.pointContainer.getNumChildren();
};
...
//    that.scene.update();
//  });
  createjs.Ticker.addEventListener('tick', that.scene);
};
