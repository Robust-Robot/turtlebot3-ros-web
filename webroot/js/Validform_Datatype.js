...
/*** 自定义 *************/
						...
						//注册用户名
						"userno":/^[a-zA-Z0-9_\u4e00-\u9fa5]{2,20}$/,
						
						//选择编码
						"sCode":/^[A-Z1-9]\d*$/,
						
						//验证:验证码
						"keyCheck":function(gets,obj,curform,datatype){
									var keyType = obj.attr("keyType");//属性,验证码类型:注册reg,登陆login.
									return CheckKey(keyType, gets);
									
								 //验证码验证
							   function CheckKey(type,gg){
									    var tmpkeynum = false;
									    $.ajax({
										    type: "POST",
										    url: "/public/SafeKeyCheck.aspx?stype="+type,
										    dataType:"text",
										    async:false,
										    success: function(key){
											    //alert("type="+type+";get="+gg+";key="+key+".");
											    if(key==gg){
												    tmpkeynum = true;
											    }else{
												    tmpkeynum = false;
